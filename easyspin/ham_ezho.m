% ham_ezho  Multiple Order Zeeman interaction Hamiltonian 
%
...
%   H = ham_ezho(SpinSystem, B, [], '', lB)
%
%
%   [G0,G1] = ham_ezho(SpinSystem)
%   [G0,G1,G2] = ham_ezho(SpinSystem)
%   [G0,G1,G2,G3] = ham_ezho(SpinSystem)
%   GlB = ham_ezho(SpinSystem, Spins, 'sparse',lB)
%   [G0,...] = ham_ezho(SpinSystem, Spins, 'sparse')
...
if TensorOutput
  ...
  if any(lb==0)
    GO = ham_ezho(SpinSystem, [0,0,0], Spins, opt,0);
  end
  xyz = 1:3;
  if any(lb==1)
    for n = 3:-1:1
  ...
  if any(lb==3)
    ...
    me = 1/len*(ham_ezho(SpinSystem, Field, Spins, opt,3)-dif);
    for k = 1:len
      G3{ind(k,1),ind(k,2),ind(k,3)} = me;
    end
  end
